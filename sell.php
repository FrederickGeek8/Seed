...
if(isset($_GET['q'], $_GET['s'])){
	...
	if($_GET['q'] <= 0){
		die("You have to sell one or more shares.");
	}
	$query = $_GET['s'];
	$success = false;
	for ($i=1;$i<=10;$i++) {
	...
	if($marketclosed){
		if(after_market_sell($_SESSION['id'], $final["price"], $_GET['s'], $final["name"], $_GET['q']) === false){
			die('You do not have enough stock to sell.');
		}
		log_change($_SESSION['id'], $_SESSION['email'], $_GET['s'], $_GET['q'], $final["price"], false);
	}else{
		if(sell_stock($_SESSION['id'], $final["price"], $_GET['s'], $_GET['q']) === false){
			die('You do not have enough stock to sell.');
		}
		log_change($_SESSION['id'], $_SESSION['email'], $_GET['s'], $_GET['q'], $final["price"], false, false);
	}
