...
if(isset($_SESSION['email']) === false){
	...
	if(isset($_REQUEST['q'], $_REQUEST['s']) && !empty($_REQUEST['q']) && !empty($_REQUEST['s'])){
		...
		if($marketclosed){
			...
			if(after_market_buy($_SESSION['id'], $final["price"], $_REQUEST['s'], $final['name'], $_REQUEST['q']) === false){
				$status['status'] = "err_money";
				echo json_encode($status);
				die();
			}
			log_change($_SESSION['id'], $_SESSION['email'], $_REQUEST['s'], $_REQUEST['q'], $final["price"]);
			
		}else{
		
			if(buy_stock($_SESSION['id'], $final["price"], $_REQUEST['s'], $final['name'], $_REQUEST['q']) === false){
				$status['status'] = "err_money";
				echo json_encode($status);
				die();
			}
			log_change($_SESSION['id'], $_SESSION['email'], $_REQUEST['s'], $_REQUEST['q'], $final["price"], true, false);
		}
