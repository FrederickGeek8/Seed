...
if(isset($_GET['success2']) || isset($_GET['active'])){
  $errors[] = 'Your account has been successfully registered.';
}
  ...
  <body>
    ...
    <h1 class='stease'>CAIC</h1>
      <form class="form-signin" style="text-align:center;" action='#' method='post'>
        <h1>Login</h1>
        <br />
        <p><input type="email" class="input-block-level" placeholder="Email address" name='email' value='<?php if(isset($_POST['email'])){ echo html_escape($_POST['email']); } ?>' required></p>
		<p><input type="password" class="input-block-level" placeholder="Password" name='password' required></p>
		...
		<?php } ?>
        <button class="btn btn-default btn-lg btn-primary" type="submit">Sign in</button>
      </form>
