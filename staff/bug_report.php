...
if(isset($_GET['resolve'])){
	if(resolve_bug($_GET['resolve']) === false){
		die('Invalid bugreport id.');
	}else{
  ...
  <body>
	...
	<div class="container-narrow">
	  ...
	  <div class="accordion" id="accordion2">
		<?php $index = -1; foreach($reports AS $report){ $index++; $infohash = sha1($report['report_id']); ?>
		<div class="accordion-group">
			 <div class="accordion-heading">
			<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#<?=$infohash?>">
			  <?php echo $infohash." - <b>{$report['report_email']}</b>"; ?>
			</a>
			 </div>
			 <div id="<?=$infohash?>" data-remote="#<?=$infohash?>" class="accordion-body collapse">
			<div class="accordion-inner">
			  <?php echo $report['report_body']; ?><a href="?resolve=<?=$report['report_id']?>" style='float:right;' class="btn btn-success btn-mini">Resolve</a>
			</div>
			 </div>
		</div>
		<?php } ?>
	...
	<script type="text/javascript">if (window.location.hash != null){$(window.location.hash).collapse()}</script>
