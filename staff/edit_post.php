...
if(isset($_GET['p']) === false){
  die('You must specify a post id.');
}else{
...
if(isset($_POST['title'], $_POST['code'])){
  ...
  if(empty($errors)){
    edit_post($_GET['p'], $_POST['title'], bbcode($_POST['code']));
  ...
  <body>
    ...
    <div class="container-narrow">
      ...
      <form class="form-post" id='formling' action='#' method='post'>
        <p><input type="text" class="input-large" placeholder="Title" name='title' value='<?php if(isset($_POST['title'])){ echo html_escape($_POST['title']); }else{ echo $post['title']; } ?>' required></p>
        <p><small>BBCode is supported in posting.</small></p>
        <p><textarea style='width:800px;height:350px;color:black;' name='code' required><?php echo html_to_bbcode($post['body']); ?></textarea></p>
        <p><button class="btn btn-large btn-primary" type="submit">Post</button>&emsp;<button id='preview' type='submit' formaction='preview.php' formtarget='_blank' class="btn btn-large btn-success">Preview</button></p>
      </form>
      <hr>
