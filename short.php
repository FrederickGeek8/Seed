...
if(isset($_GET['q'], $_GET['s'])){
	...
	if($_GET['q'] <= 0){
		die("You have to short one or more shares.");
	}
	$query = $_GET['s'];
	$success = false;
	for ($i=1;$i<=10;$i++) {
	...
	if($marketclosed){
		if(after_market_short($_SESSION['id'], $final["price"], $_GET['s'], $final['name'], $_GET['q']) === false){
			die('You do not have enough money to short this stock.');
		}
		log_change($_SESSION['id'], $_SESSION['email'], $_GET['s'], $_GET['q'], $final["price"], "short");
	}else{
		if(short_stock($_SESSION['id'], $final["price"], $_GET['s'], $final['name'], $_GET['q']) === false){
			die('You do not have enough money to short this stock.');
		}
		log_change($_SESSION['id'], $_SESSION['email'], $_GET['s'], $_GET['q'], $final["price"], "short", false);
	}
